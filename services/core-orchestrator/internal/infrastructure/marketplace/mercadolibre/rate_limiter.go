...
// RateLimiter caps how many MercadoLibre API calls may be made within a
// sliding window, regardless of endpoint (auth, categories, items, etc.). A
// single instance is meant to be shared across every Client built for
// MercadoLibre calls, since MercadoLibre enforces its own per-account rate
// limit independent of which endpoint is being invoked.

...
public interface ItemInventLocationRepository extends JpaRepository<ItemInventLocation, ItemInventLocationId> {
    ...
    @Query(value = "select Articulo, SUM(Disponible) as Total from ItemInventLocation where Almacen IN :warehouses and Articulo IN :items GROUP BY Articulo", nativeQuery = true)

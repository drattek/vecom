...
public class MSBProductSyncService {
   ...
   /* @Transactional(readOnly = false)
    public void processProducts() throws InvalidAlgorithmParameterException, NoSuchPaddingException, IllegalBlockSizeException,
            ...
            for (Iterator<Object[]> it = productsStream.iterator(); it.hasNext(); ) {
               ...
               try {
                    ...
                    if(url != "not synchronize"){
                        String response = synchronizeProducts(accessToken, product, url, synchronizedProduct);
                        updateMiddlewareSynchronizedProducts(response, synchronizedProduct);
                    } else {
                        System.out.println("The product " + synchronizedProduct.getInternalCode().toString() + " was already synchronized.");
                    }
                  // entityManager.detach(synchronizedProduct);

                    System.out.println("Cont: " + auxCont[0]); //PRUEBAS
    ...
    */
    ...
    public String processProducts() throws RuntimeException, InvalidAlgorithmParameterException, NoSuchPaddingException, IllegalBlockSizeException,
        ...
        JsonNode jsonNodeAppInfo = MiddUtils
                .getJSONResponse("An error occurred while obtaining App Information: ",
            ...
            try {
                ...
                if(url != "not synchronize"){
                    JsonNode jsonNodeSyncProducts = MiddUtils
                            .getJSONResponse("", synchronizeProducts(accessToken, product, url, synchronizedProduct));
